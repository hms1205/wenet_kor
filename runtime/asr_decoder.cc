...
DecodeState AsrDecoder::Decode(bool log, bool block) {
  return this->AdvanceDecoding(log, block);
}

void AsrDecoder::Rescoring(bool log) {
  // Do attention rescoring
  Timer timer;
  AttentionRescoring(log);
  if (log) {
    VLOG(2) << "Rescoring cost latency: " << timer.Elapsed() << "ms.";
  }
}

DecodeState AsrDecoder::AdvanceDecoding(bool log, bool block) {
...
void AsrDecoder::UpdateResult(bool log, bool finish) {
...
void AsrDecoder::AttentionRescoring(bool log) {
  searcher_->FinalizeSearch();
  UpdateResult(log); //true
