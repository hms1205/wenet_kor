...
void TorchAsrModel::WarmingUp(int warm_num) {
  wenet::Timer time_check;
  // torch::DeviceType device = at::kCUDA;
#ifdef USE_GPU
  torch::DeviceType device = at::kCUDA;
  std::cout << "warming up .. " << std::endl;
  torch::Tensor c = torch::rand({1000, 1000}, device);
  for (int i = 0; i < warm_num; i++) {
    torch::Tensor a = torch::rand({1000, 1000}, device);
    torch::Tensor b = torch::rand({1000, 1000}, device);
    c = c / (c + (a*b));
  }
  int end_time = time_check.Elapsed();
  std::cout << "done, warmup time = " << end_time << "ms." << std::endl;
#endif
}
...
void TorchAsrModel::Read(const std::string& model_path) {
  ...
  if (!torch::cuda::is_available()) {
    ...
    device = at::kCUDA;
    //device = C10_API::Device::Device(c10::DeviceType::CUDA, 1);
  }
#endif
